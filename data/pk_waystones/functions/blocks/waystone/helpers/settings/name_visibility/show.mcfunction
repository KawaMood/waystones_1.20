...
execute if score $pk.waystones.settings.name_visibility pk.value matches 1 run tellraw @s [{"text": "Name Visibility","color": "light_purple"},{"text": " setting is already set on ","color": "red"},{"text": "Show","color": "light_purple"}]
