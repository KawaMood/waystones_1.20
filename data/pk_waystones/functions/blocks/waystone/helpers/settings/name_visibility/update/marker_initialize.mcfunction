...
tag @s add pk.custom_block.update
...
execute if score $temp pk.dimension.id matches -1..1 run function pk_waystones:blocks/waystone/helpers/settings/name_visibility/update/in_vanilla_dimension/start
# - If the custom block is in custom dimension
execute if score $temp pk.dimension.id matches 100.. run function pk_waystones:blocks/waystone/helpers/settings/name_visibility/update/in_custom_dimension/start